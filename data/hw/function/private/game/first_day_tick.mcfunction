...
function hw:private/game/first_day_tick_timer with storage hw:tmp timer

execute if score #day_counter hw matches ..0 run return -1

#update timer
scoreboard players remove #day_counter hw 1
execute if score #day_counter hw matches ..0 run function hw:private/game/schedule_enter_night